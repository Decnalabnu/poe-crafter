# DEPRECATED 2026-04-22: superseded by fetch_stacked_decks_weights.py, which
# pulls empirical 3.28 drop data from the community stacked-decks spreadsheet
# and uses these Faustus-gold-cost heuristics only as a fallback for cards the
# sheet does not cover. Running this standalone would overwrite card_weights.json
# with Faustus-only data and drop the sheet-only cards, so the body is disabled.
#
# import json
# import os
#
...
if __name__ == "__main__":
    raise SystemExit(
        "generate_faustus_weights.py is deprecated. "
        "Run src/utils/fetch_stacked_decks_weights.py instead."
    )
